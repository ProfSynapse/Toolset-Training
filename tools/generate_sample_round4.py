#!/usr/bin/env python3
"""
Generate Round 4 sampling batches (batches 16-30) - 15 batches of 30 examples each.
Avoids all previously sampled examples from rounds 1-3.
"""
...
def load_previous_samples():
    """Load all previously sampled line numbers from rounds 1-3."""
    sampled = set()
    review_dir = Path("Datasets/quality_review")

    # Load from previous manifests
    for i in range(1, 16):  # Batches 1-15 from rounds 1-3
...
def main():
    ...
    num_batches = 15  # Batches 16-30
    batch_size = 30
    batch_start_num = 16

    print(f"Round 4 Batch Generation")

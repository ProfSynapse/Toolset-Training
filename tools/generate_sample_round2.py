#!/usr/bin/env python3
"""
Generate second round of 150 samples from the remaining unscored examples.
Avoids duplicating examples from round 1.
"""
import json
import sys
...
def main():
    ...
    manifest_path = review_dir / "sample_manifest.json"

    if not dataset_path.exists():
        sys.exit(f"Dataset not found: {dataset_path}")

    # Load round 1 manifest to get already-sampled line numbers
    already_sampled = set()
    ...
    print(f"Round 1 sampled: {len(already_sampled)} examples")
    ...
    # Create 5 batches of 30 examples each (batches 6-10)
    batch_size = 30
    batches = []
    for i in range(5):
    ...
    for batch_num, batch in enumerate(batches, start=6):  # Start at batch 6
        ...
        manifest_round2["batches"].append(batch_info)
    ...
    # Write round 2 manifest
    manifest_file = review_dir / "sample_manifest_round2.json"
    with open(manifest_file, 'w', encoding='utf-8') as f:
        json.dump(manifest_round2, f, indent=2)

    print(f"\n✓ Generated {sample_size} new samples across 5 batches (6-10)")
    ...
    print(f"  Round 1: {len(already_sampled)}")
    print(f"  Round 2: {sample_size}")

#!/usr/bin/env python3
"""
Generate Round 6 sampling batches (batches 61-120) - 60 batches of 30 examples each.
Avoids all previously sampled examples from rounds 1-5.
"""
...
def load_previous_samples():
    """Load all previously sampled line numbers from rounds 1-5."""
    sampled = set()
    review_dir = Path("Datasets/quality_review")

    # Load from previous manifests (batches 1-60)
    for i in range(1, 61):
...
def main():
    ...
    num_batches = 60  # Batches 61-120
    batch_size = 30
    batch_start_num = 61

    print(f"Round 6 Batch Generation - 60 PARALLEL AGENTS!")
